...
class AuthorizationReceived extends StatelessWidget {
  const AuthorizationReceived({super.key});
    ...
    return BlocConsumer<AuthorizationScreenBloc, AuthorizationScreenState>(
      ...
      builder: (context, state) {
        return Container(
          ...
          child: state.listOfAuthorizationsReceived.isNotEmpty
              ? CustomScrollView(
                  slivers: [
                    SliverToBoxAdapter(
                      child: Column(
                        children: [
                          CustomSubtitleBox(
                            subtitle: context
                                .localizations.petition_received_tab_subtitle,
                          ),
                        ],
                      ),
                    ),
                    SliverList(
                      delegate: SliverChildBuilderDelegate(
                        childCount: state.listOfAuthorizationsReceived.length,
                        (context, index) {
                          AuthDataEntity data =
                              state.listOfAuthorizationsReceived[index];

                          AuthSenderUseEntity user =
                              state.listOfAuthorizationsReceived[index].user ??
                                  AuthSenderUseEntity(
                                    id: '',
                                    dni: '',
                                    username: '',
                          ...
                          String? endDate =
                              state.listOfAuthorizationsReceived[index].revDate;

                          AuthorizationStatus status = getAuthorizationStatus(
                            state.listOfAuthorizationsReceived[index].state ??
                                '',
                          );
                            ...
                            title: AFMenu.normal(
                              ...
                              information: endDate != null
                                  ? context
                                      ...
                                      formattedDateDayMonthYear(endDate)
              ...
              : EmptyAuthorizationSection(
                  afTitle: AFTitle(
                    ...
                    title:
                        context.localizations.petition_received_alert_message,
                    size: AFTitleSize.s,

...
void isCheckBoxSelected(BuildContext context, state, RequestEntity request) {
  final previousValue = state.selectedRequests[request] ?? false;
  context.read<MultiSelectionRequestBloc>().add(
        MultiSelectionRequestEvent.isCheckBoxSelected(
          !previousValue,
          request,
        ),
      );
